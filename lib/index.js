...
export async function readMarkdown(folderOrFilePath) {
    const mdItems = [];
    ...
    if (!stats.isDirectory()) {
        return gatherMarkdownData(folderOrFilePath);
    }

    const res = globSync(`${folderOrFilePath}/**/*.md`);

    res.forEach((mdFilePath) => mdItems.push(gatherMarkdownData(mdFilePath)));
